...
def check_url(encoded_id):
    ...
    try:
        ...
        visibility_element = soup.find('div', class_='System-privateText')
        title = title_element.text.strip() if title_element else "Untitled map"
...
for i in range(1000):
    ...
    if result:
        ...
        print(f"TITLE: {title} ({visibility})")
        misses = 0
        found_maps.append((i, f"{BASE_URL}{encoded}", title, visibility))
    else:
        print("NONE FOUND")
        misses += 1
...
with open(html_filename, "w", encoding="utf-8") as f:
    ...
    f.write("<table>\n<tr><th>ID</th><th>Title</th><th>Visibility</th><th>Link</th></tr>\n")
    for idx, url, title, visibility in found_maps:
        f.write(f"<tr><td>{idx}</td><td>{title}</td><td>{visibility}</td><td><a href='{url}' target='_blank'>{url}</a></td></tr>\n")
    f.write("</table>\n</body>\n</html>")
